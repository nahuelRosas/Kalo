...
const AdmOrders: React.FC<AdmOrdersProps> = () => {
  ...
  return (
    <>
      {orders.map((order, index) => {
        ...
        const _User = usersData?.find(
          (user) => user.email === order.charges.data[0].billing_details.email
        );
        const goReceipt = () => {
            ...
            <Flex
              ...
              <Grid templateColumns="repeat(2, 2fr)" gap={3} bg={bg} w={"100%"}>
                <Box m={4}>
                  ...
                  <Text>{_User?.address.line1}</Text>
                  <Text>
                    {_User?.address?.city}, {_User?.address?.postal_code}
                  </Text>
                  <Text>{_User?.address?.country}</Text>
                </Box>
                <Box m={2}>
                  <Text fontSize={"xl"} fontWeight={"bold"}>
                    Order Summary
